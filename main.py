...
# Add your code below!
bot_moves = Counter(move[0] for move in robot_moves)


# Count the number of collisions by robot name
# Add your code below!
bot_collides = Counter(move[0] for move in robot_moves if move[2] is True)
...
for bot in bots:
    bot_scores.append(
        BotScoreData(
            bot.name,
            bot_moves[bot.name],
            bot_collides[bot.name],
            bot_moves[bot.name] + bot_collides[bot.name],
        )
    )
