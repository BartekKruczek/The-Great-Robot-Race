...
def print_results(bot_score_data):
    ...
    for score_data in bot_score_data:
        ...
        print(
            ...
            + " Moves: "
            + str(score_data.num_moves)
            + " Collisions: "
            + str(score_data.num_collisions)
        )
        place += 1
...
def compute_robot_logic(walls, goal, bot):
    ...
    for wall in walls:
        if (
            bot.calc_x + selected_move[0] == wall.x
            and bot.calc_y + selected_move[1] == wall.y
        ):
            hit_wall = True
    ...
    if (
        bot.calc_x + selected_move[0] == goal.x
        and bot.calc_y + selected_move[1] == goal.y
    ):
